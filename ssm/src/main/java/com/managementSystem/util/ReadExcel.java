...
//                        if(existID.contains(id)) continue;
    ...
    public List<Student_Course> getStudentId(String fileName, MultipartFile Mfile){
        ...
        try{
            ...
            is = new FileInputStream(file1);
            //根据excel里面的内容读取客户信息
            UserList = getStudentId(is, isExcel2003);
            is.close();
        }catch(Exception e){
            e.printStackTrace();
        } finally{
     ...
     * @throws IOException
     */
    public  List<Student_Course> getStudentId(InputStream is,boolean isExcel2003){
        List<Student_Course> UserList=null;
        try{
            /** 根据版本选择创建Workbook的方式 */
            Workbook wb = null;
     ...
     * @return
     */
    private List<Student_Course> getStudentId(Workbook wb){
        //得到第一个shell
        Sheet sheet=wb.getSheetAt(0);
